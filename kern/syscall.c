...
sys_cputs(const char *s, size_t len)
{
	...
	cprintf("sys_cputs address is %p\n", s);
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// Check env exist and with with permission to change
	if (envid2env(envid, &env, 1) < 0)
		return -E_BAD_ENV;

	// Check va is under UTOP
	...
	if ((pp = page_alloc(ALLOC_ZERO)) == NULL) // not enough memory
		return -E_NO_MEM;
